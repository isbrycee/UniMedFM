...
class LoRALayer(torch.nn.Module):
    def __init__(self, in_dim, out_dim, rank, alpha):
        ...
        self.alpha = alpha
        self.text_embed_proj = nn.Linear(768, 256)
        self.norm = nn.LayerNorm(256)
        self.proj_k = nn.Linear(256, rank)
        self.proj_v = nn.Linear(256, rank)
        self.cross_attn = nn.MultiheadAttention(rank, 1, batch_first=False)
...
class TwoWayAttentionBlock3D(nn.Module):
    def __init__(
        ...
        # add by bryce
        # self.lora_k_i2t = LoRALayer(embedding_dim, embedding_dim, 64, 0.25)
        # self.lora_q_i2t = LoRALayer(embedding_dim, embedding_dim, 64, 0.25)
        # self.lora_k_t2i = LoRALayer(embedding_dim, embedding_dim, 64, 0.25)
        # self.lora_q_t2i = LoRALayer(embedding_dim, embedding_dim, 64, 0.25)
    ...
    def forward(
        ...
        k = keys + key_pe
        # add by bryce
        # q = self.lora_q_t2i(q,text_embed)
        # k = self.lora_k_t2i(k,text_embed)
...
class MaskDecoder3D(nn.Module):
    ...
    def predict_masks(
        ...
        hs, src = self.transformer(src, pos_src, tokens, text_embed)
        iou_token_out = hs[:, 0, :]
