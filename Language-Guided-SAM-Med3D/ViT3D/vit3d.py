...
class DeiT_Transformer3D(nn.Module):
    ...
    def __init__(self, 
        ...
        self.mlp = nn.Sequential(nn.Linear(embed_dim, 512),
                                  nn.ReLU(),
                                  nn.Linear(512, 256),
                                  nn.ReLU(),
                                  )
    ...
    def forward(self, x, M3D_CLIP_visual_features=None):
        """
        Input
        -----
        Shape (n_samples, in_chans, img_size, img_size)
        ...
        # cat_features = torch.cat((cls_token_final, M3D_CLIP_visual_features), dim=-1)
        x = self.head(self.mlp(cls_token_final))
        # x = self.head(cls_token_final)
