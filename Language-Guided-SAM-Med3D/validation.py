...
if __name__ == "__main__":    
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if(args.skip_existing_pred and os.path.exists(pred_path)):
            ...
            out = sitk.GetImageFromArray(seg_mask_list[best_id])
            os.makedirs(os.path.join(vis_root, 'best'), exist_ok=True)
            sitk.WriteImage(out, os.path.join(vis_root, 'best', os.path.basename(img_name[0]).replace(".nii.gz", f"_pred_best.nii.gz")))
